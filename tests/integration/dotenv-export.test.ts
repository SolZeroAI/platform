...
const parseAssignment = (assignment: string) =>
  Effect.runSync(
    Config.string("C0_CONFIG_TEST").parse(ConfigProvider.fromDotEnvContents(assignment)),
  )
...
describe("dotenv export", () => {
  it.each([
    ...
    const assignment = Effect.runSync(dotenvAssignment("C0_CONFIG_TEST", value))
  ...
  it("uses an unquoted value when it is the only lossless representation", () => {
    ...
    expect(assignment).toBe(`C0_CONFIG_TEST=${value}`)
    expect(parseAssignment(assignment)).toBe(value)
  })
  ...
  it("fails instead of emitting a value that dotenv would change", () => {
    expect(() =>
      Effect.runSync(
        dotenvAssignment("C0_CONFIG_TEST", "single ' double \" backtick ` # comment\nnext line"),
      ),
  ...
  it.each(["first line\rsecond line", "first line\r\nsecond line"])(
    ...
    (value) => {
      expect(() => Effect.runSync(dotenvAssignment("C0_CONFIG_TEST", value))).toThrow(
  ...
  it("rejects variable names that could create another assignment", () => {
    expect(() => Effect.runSync(dotenvAssignment("C0_CONFIG_TEST\nINJECTED", "value"))).toThrow(
